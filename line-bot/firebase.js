...
const FIREBASE_URL = 'https://trade-in-5135c-default-rtdb.asia-southeast1.firebasedatabase.app/tradein_prices/prices.json';

function baseUrl() {
  return 'https://trade-in-5135c-default-rtdb.asia-southeast1.firebasedatabase.app';
}
...
async function getLastKnownPrices() {
  const res = await fetch(`${baseUrl()}/epbox_last_known_prices.json`);
  if (!res.ok) throw new Error(`Firebase 回應錯誤：${res.status}`);
